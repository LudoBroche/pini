...
class HDF5Widget(qt.QWidget):
    ...
    def __init__(self, hdf):
        ...
        self.tree_view = qt.QTreeView(headerHidden=False)
        self.tree_view.setSelectionBehavior(qt.QAbstractItemView.SelectRows)
        self.tree_view.setEditTriggers(qt.QAbstractItemView.NoEditTriggers)
        self.tree_view.setModel(self.tree_model)

        self.tree_view.header().setSectionResizeMode(0, qt.QHeaderView.Interactive)
        self.tree_view.header().resizeSection(0, 160)
        self.tree_view.header().setSectionResizeMode(1, qt.QHeaderView.ResizeToContents)
        self.tree_view.header().setStretchLastSection(True)

        # Setup attributes table view
        self.attrs_view = qt.QTableView()
        self.attrs_view.setModel(self.attrs_model)
        self.attrs_view.horizontalHeader().setSectionResizeMode(qt.QHeaderView.Stretch)
        self.attrs_view.horizontalHeader().setStretchLastSection(True)
        self.attrs_view.verticalHeader().hide()

        # Setup dataset table view
        self.dataset_view = qt.QTableView()
        self.dataset_view.setModel(self.dataset_model)
        self.dataset_view.horizontalHeader().setSectionResizeMode(qt.QHeaderView.Stretch)
        self.dataset_view.horizontalHeader().setStretchLastSection(True)
        self.dataset_view.verticalHeader().hide()

        # Setup dims table view
        self.dims_view = qt.QTableView()
        self.dims_view.setModel(self.dims_model)
        self.dims_view.horizontalHeader().setSectionResizeMode(qt.QHeaderView.Stretch)
        self.dims_view.horizontalHeader().setStretchLastSection(True)
        self.dims_view.verticalHeader().hide()


        # Setup main data table view
        self.data_view = qt.QTableView()
        self.data_view.setModel(self.data_model)

        # Setup tabs
        self.tabs = qt.QTabWidget()
        self.tabs.setTabPosition(qt.QTabWidget.South)
        self.tabs.setTabsClosable(True)

        self.tabs.addTab(self.data_view, 'Table')

        self.tabs.tabBar().tabButton(0, qt.QTabBar.RightSide).deleteLater()
        self.tabs.tabBar().setTabButton(0, qt.QTabBar.RightSide, None)
    ...
    def handle_dims_data_changed(self, topLeft, bottomRight, roles):
        ...
        if isinstance(self.tabs.currentWidget(), qt.QTableView):
    ...
    def handle_selection_changed(self, selected, deselected):
        ...
        path = self.tree_model.itemFromIndex(index).data(qt.Qt.UserRole)
    ...
    def handle_tab_changed(self):
        ...
        self.dims_model.dataChanged.emit(qt.QModelIndex(), qt.QModelIndex(), [])
...
class ImageView(qt.QAbstractItemView):
    """
    ...
    def __init__(self, model, dims_model):
        ...
        self.scrollbar = qt.QScrollBar(qt.Qt.Horizontal)
    ...
    def handle_mouse_moved(self, pos):
        ...
        if self.image_item.image is not None and len(self.model().dims) >= 2:
            ...
            if 0 <= x < max_x and 0 <= y < max_y:
                ...
                self.viewbox.setCursor(qt.Qt.CrossCursor)
            else:
                self.window().status.showMessage('')
                self.viewbox.setCursor(qt.Qt.ArrowCursor)
