...
class TreeModel(qt.QStandardItemModel):
    ...
    def add_node(self, parent_item, node):
        ...
        tree_item = qt.QStandardItem(node_name)
        tree_item.setData(node_path, qt.Qt.UserRole)
        tree_item.setToolTip(node_path)

        num_attrs = len(node.attrs)
        if num_attrs > 0:
            attrs_item = qt.QStandardItem(str(num_attrs))
        else:
            attrs_item = qt.QStandardItem('')

        attrs_item.setForeground(qt.QBrush((qt.Qt.darkGray)))

        if isinstance(node, h5py.Dataset):
            tree_item.setIcon(qt.QIcon('./lib/resources/images/dataset.svg'))
            dataset_item = qt.QStandardItem(str(node.shape))

        elif isinstance(node, h5py.Group):
            tree_item.setIcon(qt.QIcon('./lib/resources/images/folder.svg'))
            dataset_item = qt.QStandardItem('')

        dataset_item.setForeground(qt.QBrush((qt.Qt.darkGray)))
    ...
    def handle_expanded(self, index):
        ...
        item.setIcon(qt.QIcon('./lib/resources/images/folder-open.svg'))
        ...
        for row in range(item.rowCount()):
            ...
            path = child_item.data(qt.Qt.UserRole)
    ...
    def handle_collapsed(self, index):
        ...
        """
        item = self.itemFromIndex(index)
        item.setIcon(qt.QIcon('./lib/resources/images/folder.svg'))


class AttributesTableModel(qt.QAbstractTableModel):
    ...
    def data(self, index, role=qt.Qt.DisplayRole):
        ...
        if index.isValid():
            ...
            if role in (qt.Qt.DisplayRole, qt.Qt.ToolTipRole):
                if column == 0:
...
class DatasetTableModel(qt.QAbstractTableModel):
...
class DataTableModel(qt.QAbstractTableModel):
    ...
    def headerData(self, section, orientation, role):
        if role == qt.Qt.DisplayRole:
            if self.compound_names and orientation == qt.Qt.Horizontal:
...
class ImageModel(qt.QAbstractItemModel):
    ...
    def parent(self, childIndex=qt.QModelIndex()):
        return self.createIndex()

    def index(self, row, column, parentIndex=qt.QModelIndex()):
        return self.createIndex()
...
class DimsTableModel(qt.QAbstractTableModel):
