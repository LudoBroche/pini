...
class SliceVisualizer(qt.QWidget):
    ...
    def display_image(self):
        ...
        if self.flagAlpha:
            imageAlpha = qt.QImage(self.dataAlpha, self.dataAlpha.shape[1], self.dataAlpha.shape[0],
                                   self.dataAlpha.shape[1], qt.QImage.Format_Indexed8)
    ...
    def drawPointPolygon(self, x, y, FlagEnd):
        ...
        if not FlagEnd:
            ...
            self.scene.addLine(self.posPolyPoints[0][0], self.posPolyPoints[-0][1], self.posPolyPoints[-1][0],
                               self.posPolyPoints[-1][1], qt.Qt.green)
...
class Interactor3D(qt.QWidget):
    ...
    def updateOverlays(self, Overlay):
        ...
        self.axialWidget.add_overlay(Overlay["Flag"], Overlay["Image"], Overlay["Alpha"], Overlay["ColorMap"],
                                     Overlay["Range"][0], Overlay["Range"][1])
        self.coronalWidget.add_overlay(Overlay["Flag"], Overlay["Image"], Overlay["Alpha"], Overlay["ColorMap"],
                                       Overlay["Range"][0], Overlay["Range"][1])
        self.sagittalWidget.add_overlay(Overlay["Flag"], Overlay["Image"], Overlay["Alpha"], Overlay["ColorMap"],
                                        Overlay["Range"][0], Overlay["Range"][1])
